...
def scrape_kalibrr(keywords):
    ...
    # Write raw DOM content to file
    with open('rawDOM.html', 'w', encoding='utf-8') as file:
        file.write(DOM_content)
    print("Raw DOM content has been written to 'rawDOM.html'")
...
def clean_dom(DOM_content):
    ...
    # Tags to remove
    tags_to_remove = ['script', 'style', 'iframe', 'noscript', 'head', 'meta', 'link', 'header', 'footer', 'nav', 'svg', 'path']
                ...
                for element in job_div.children:
                    if element.name == 'a' and element.get('href'):
                        ...
                        for link in element.find_all('a', href=True):
                            ...
                            if link.get_text(strip=True) == "View Post" and link['href']:
                                job_content.append(f"{base_URL}{link['href']}")
                if job_link:
